...
- Lean mode is the default local-only tool policy (`PI_CURSOR_LEAN` unset or `1`). It forces the Pi bridge and overlapping builtin exposure on, forces `local.settingSources: []`, and passes SDK `tools: ["mcp"]` on both create and resume (`tools: []` for an empty bridge). Only the internal Pi MCP endpoint is supplied. Native web search, host tools, and subagents are unavailable. Pi's active registry is authoritative; replay wrappers, question/skill activation tools, and skill catalog rewriting are disabled, preserving Pi instructions and skills. The pool/resume identity includes lean mode so an unrestricted agent cannot be reused. Cloud fails before agent creation. Set `PI_CURSOR_LEAN=0` at startup to opt out or use Cloud, and restart to change it. The following legacy tool-surface behavior applies when lean mode is off.
- Lean mode passes `local.settingSources: []` by default, so Cursor MCP servers, plugin tools, project/user settings, and related Cursor-native capabilities are not loaded. `PI_CURSOR_LEAN=0` restores the legacy setting-source behavior, which defaults to `settingSources: ["all"]` and supports `PI_CURSOR_SETTING_SOURCES=project,user,plugins` or `none`. `/cursor-refresh-config` calls the current pooled SDK agent's `agent.reload()` to refresh filesystem Cursor config without recreating the agent. The provider suppresses direct Cursor SDK bootstrap stdout/stderr/console noise.
